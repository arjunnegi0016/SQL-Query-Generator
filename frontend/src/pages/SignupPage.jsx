...
export default function SignupPage() {
  ...
  // Step 1: User Details, Step 2: OTP
  ...
  const handleSignupSubmit = async (e) => {
    ...
    // Validation
    if (!formData.name || formData.name.length < 2) return setError("Name must be at least 2 characters long.");
    ...
    setLoading(true);
    try {
      const res = await axios.post('http://localhost:5000/api/auth/signup-request', {
        name: formData.name,
      ...
      setError(err.response?.data?.error || "Failed to initiate signup. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-md space-y-8 p-8 bg-card rounded-2xl shadow-xl border border-border transition-all duration-300">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {step === 1 ? 'Sign up to get started' : `We sent a code to ${formData.email}`}
        </p>
      </div>
